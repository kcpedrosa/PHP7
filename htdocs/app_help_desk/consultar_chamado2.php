...
<?php
  ...
  while(!feof($arquivo)) { //testa pelo fim de um arquivo
    //linhas  
    //the last data that fgets adds is an empty element
    ...
    //the explode function give us an array with numeric keys
    

    //(perfil id = 2) só vamos exibir o chamado, se ele foi criado pelo usuário
    if($_SESSION['perfil_id'] == 2) {
      //perfil_id == 2 means that it is a common user
      ...
      $chamados[] = $registro; //if the user is not perfil_id == 1, then he is 1 and by that he's
      //an admin, so it will add all the registers, even the empty one
      /*
      echo '<pre>';
      var_dump($chamados);
      echo '</pre>';
      //it will add the line one by one in $chamados,first entry will have one line,secod two etc
      */    
    }
            ...
            <div class="card-body">
              ...
              <? foreach($chamados as $chamado) { ?>
                ...
                <?php
                  ...
                  /*
                  //it is adding correctly depending on the profile
                  echo '<pre>';
